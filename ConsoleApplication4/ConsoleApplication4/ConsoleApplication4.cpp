...
display(void)
{
	...
	//Modification: added one more ico
	glClear(GL_DEPTH_BUFFER_BIT);
	glEnable(GL_LIGHT3);
	glDisable(GL_LIGHT1);
	glDisable(GL_LIGHT2);
	...
	glRotatef((angle1 + angle2)/2, 0.5, 1.5, 2.0);
	glCallList(1);        /* render ico display list */
	glPopMatrix();
	//ending modification
...
idle(void)
{
	...
	int acc1 = rand()%43;
	int acc2 = rand()%37;
	angle1 = (GLfloat)fmod(angle1 + (double) acc1/500, 360.0);
	angle2 = (GLfloat)fmod(angle2 + (double) acc2/500, 360.0);
	s += 0.005;
	glutPostRedisplay();
}
